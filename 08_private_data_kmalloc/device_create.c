...
static ssize_t  my_read(struct file  *filp, char __user *user_buf, size_t len, loff_t *off)
{
	...
	pr_info("hello_cdev -read is called, we want to read %ldbytes but actually only copying %d bytes.  the offset is %lld\n ",len,to_copy, *off);
	if(*off >= sizeof(text))
		return 0;
...
static ssize_t  my_write(struct file  *filp, const  char __user *user_buf, size_t len, loff_t *off){
	...
	pr_info("hello_cdev -write is called, we want to write  %ldbytes but actually only copying %d bytes.  the offset is %lld\n ",len,to_copy, *off);
	if(*off >= sizeof(text))
		return 0;
